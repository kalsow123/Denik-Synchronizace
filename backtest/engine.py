...
class BacktestEngine:
    ...
    def run(
        ...
        from backtest.wave_sim_cache import get_cached_pine_simulation, store_pine_simulation_cache

        cached = get_cached_pine_simulation(df, cfg)
        if cached is not None:
            all_waves, wave_birth, ext_suppress_from, ext_forming_from = cached
        else:
            all_waves, wave_birth, ext_suppress_from, ext_forming_from = (
                run_pine_wave_simulation(df, cfg)
            )
            store_pine_simulation_cache(
                df, cfg, all_waves, wave_birth, ext_suppress_from, ext_forming_from,
            )
        ...
        for i in range(1, ohlc.n):
            ...
            if new_waves:
                waves_up_to_now.extend(new_waves)
                protected_waves_bar = compute_wave_2_no_tp_protected_waves(
                    waves_up_to_now, self.wave_sequence_info, cfg,
                )
            ...
            if self.trend_states_per_bar:
                ...
                if bos_flip_handler_should_run(
                    ...
                    self._handle_bos_exit_on_bar(
                        ...
                        protected_waves=protected_waves_bar,
                    )

            # Position cap varianta 2: preventivne prune pendingy bez re-queue.
            for o in apply_pending_prune(self, mid_price):
            ...
            self._trigger_pending(i, bar_time, high, low, open_)
            enforce_market_overflow(self, i, bar_time, mid_price)
            ...
            self._check_sl_tp(i, bar_time, high, low)
            ...
            # 4) Expiry pending
            self._expire_pending(i, bar_time)
    ...
    def _add_pending(self, wave: dict, order_type: str, ep: float, sl: float,
        ...
        self.wave_debug["orders_created_pending"] += 1
