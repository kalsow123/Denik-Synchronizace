...
VISUAL WAVES (PNG/HTML vln + obchodu; klice nejsou v BotConfig ani v base profilu):
  ...
  - CLI: --visual-waves , --visual-html , --visual-clip , --visual-last-n , --visual-bars , --visual-full-span
...
ABORT FIB LEVEL (pasionka / hluboký retracement — rozšíření jednoho klíče `abort_fib_level`)
  ...
  - counter_position_enabled (default False) — protipozice LIMIT v opacnem smeru
    na TP cene. Vzdy jen na TP-vlne (N, N+2, ...) dle `tp_target_wave_index`;
    `wave_target_n` = extension TP, `rrr_fixed`/`bos_exit` = RRR TP te vlny.
    Risk = cfg.risk_usd;
    SL z ladderu (viz nize) s min floorem `ext_min_sl_move_pct` (sdileno s EXT secondary).
  ...
  - two_sided_entry_enabled (default False) + two_sided_entry_min_wave_pct (default 0.55) —
    rodic A v [min, ext_wave_min_pct); B = prvni opacna Pine vlna po A (>= wave_min_pct).
    Nesplni-li FIB/EXT → nic. Vstup fib50 B, min SL 0.16 %.
    Obchazi trend_filter; primarni WAVE na B se v tom pripade neplni.
  ...
  - pp_enabled (default False) + pp_sl_pct (default 0.21) + pp_risk_usd (default 500.0) —
    ...
    neberou. Nova vlna ve smeru trendu rusi predchozi PP pending (is_pp +
  ...
  counter_position_enabled / bos_entry_enable / bos_entry_in_rrr_fixed /
  two_sided_entry_enabled / pp_enabled —
    ...
    "grid": [
        {
            ...
            "counter_position_enabled": [True],  # - set: True / False
            ...
            "ext_counter_enabled": [True], # EP v TIME - EXT_BOS# - set: True / False
            "ext_counter_time": ["23:00"],
            "ext_counter_min_sl_enabled": [True],  # min SL floor u EXT counter; False = jen ext_high/low
            "ext_counter_min_sl_pct": [0.16],  # min SL od entry v % (EXT counter time + EXT_BOS)
            "ext_bos_enabled": [True], # - set: True / False
    ...
    "grid": [
        {
            ...
            "counter_position_enabled": [True, False],
...
def _grid_bot_name_extra_short() -> dict:
    ...
    return {
        ...
        "ext_bos_enabled": "ext_bos_enabled",
...
def build_grid_bot_name_from_cfg(cfg: dict, keys: list[str]) -> str:
    ...
    bool_skip_when_false = {
        ...
        "counter_position_enabled",    # WAVE_TARGET_N: protipozice na TP cene
        ...
        "two_sided_entry_enabled",     # TWO-SIDED counter (doplnkovy vstup)
        "wave_position_enabled",       # klasické vlny (fib vstupy z vlny)
