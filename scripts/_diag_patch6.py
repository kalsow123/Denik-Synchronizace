...
def compute_wave_2_no_tp_protected_waves(
    ...
    VÝJIMKA: Pokud je druhá vlna v pořadí (index 2) EXT vlna, ochrana
    pro celý trend NEPLATÍ.
    """
        ...
        while curr_node:
            ...
            if node_info and node_info.index_in_trend is not None:
                ...
                if idx > trend_max_idx:
                    trend_max_idx = idx
                if idx == 2 and waves_by_wt[curr_node].get("is_ext"):
                    has_ext_idx_2 = True
                    
            curr_node = next_in_trend.get(curr_node)
            
        # Pokud trend nepresahl max_idx a zaroven neni zneplatnen EXT vlnou na indexu 2
        if trend_max_idx <= max_idx and not has_ext_idx_2:
...
def _ghost_skip_wave(
    ...
    if _wave_is_visible(w, cfg, hh_hl_filter):
        return False
                if str(wt) == "202505122230":
                    print(f"[{df.iloc[i][\'time\']}] Assigning result[wt] at line XYZ")
...
def propagate_seq_info_to_waves(
    ...
    """
    Propaguje hodnoty z seq_info zpět do wave dict.

    Po volání compute_wave_sequence_info_per_wave musí každá wave mít:
      - w["index_in_trend"] (Optional[int])
      - w["is_bos_wave"] (bool)
      - w["prev_same_dir_in_trend_wave_time"] (Optional[str])

    Důvod: HTML vykreslovač čte tyto hodnoty z wave dict (ne ze seq_info).
    Idempotent: druhé volání se stejnými vstupy = identický výsledek.
    """
    for w in waves:
    ...
    *,
    ...
    for w in waves:
        ...
        if info is None or info.index_in_trend is not None:
            continue
        if w.get("post_ext_trend_suppressed") or w.get("is_two_sided_counter"):
            continue
    ...
    if not cands:
        return None
    _dr, wt, wdir = min(cands, key=lambda c: c[0])
    return wt, wdir
...
def compute_wave_sequence_info_per_wave(df: pd.DataFrame,
    ...
    okno (`ext1_count_window`). Engine ho pouziva pro ochranu pozic: behem
    ...
    for i in range(n):
        ...
        if ext_active_wave is not None:
            if check_close_breaks_ext_extreme(bar_close, ext_active_wave):
                ...
                ext1_count_window = False  # Vracíme zpět: uživatelův test vyžaduje vypnutí po dosažení high
                ext1_count_window = False  # Vracíme zpět: uživatelův test vyžaduje vypnutí po dosažení high
        ...
        for w in new_waves:
            wt = str(w["wave_time"])
            if wt == "202505140130":
                print(f"Inside compute loop: is_ext={w.get('is_ext')}")
            wdir = int(w["dir"])
        
            if str(wt) == "202505122230":
                print(f"[{df.iloc[i]['time']}] Before processing Wave 202505122230: state.direction={state.direction}")
            ...
            if (w.get("hh_hl_pass") is False) and not is_ext:
                ...
                if state.direction != "neutral" and not wave_is_trend_dir and not is_reversal:
                if str(wt) == "202505122230":
                    print(f"[{df.iloc[i][\'time\']}] Assigning result[wt] at line XYZ")
                    result[wt] = WaveSequenceInfo(None, None)
                    continue
            ...
            if state.is_bos_wave_pending:
                ...
                if wave_dir_matches_flip:
                    ...
                    if wdir == 1:
                        ...
                        counter_up = 0
                        last_same_dir_up_wt = None
                    if str(wt) == "202505140130":
                        print(f"Inside is_bos_wave_pending: ext1_count_window={ext1_count_window}, ext1_counter_idx={ext1_counter_idx}, new_idx={new_idx}")
                if str(wt) == "202505122230":
                    print(f"[{df.iloc[i][\'time\']}] Assigning result[wt] at line XYZ")
            ...
            # Klasický BOS check (vlna sama je BOS, ne EXT)
            is_bos_wave = False
            ...
            # Reverzni vlna po trend-dir EXT (climax): PRVNI opacna vlna dostane
            ...
            if ext_active_wave is not None:
                ...
                if wave_is_counter:
                    ...
                    if str(wt) in ["202505122230", "202505130800"]:
                        print(f"[{df.iloc[i]['time']}] UP wave {wt}: ext1_count_window={ext1_count_window}, ext1_counter_idx={ext1_counter_idx}")
                    if ext1_count_window:
                        if _ghost_skip_wave(w, cfg, hh_hl_filter, result, wt):
                            continue
                        ext1_counter_idx += 1
        ...
        if window_out is not None:
            window_out.append(bool(ext1_count_window))
